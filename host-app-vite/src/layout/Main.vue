<template>
  <QLayout
    :class="`app-main-layout ${mobileView ? 'mobile-view' : ''}`"
    view="hHh lpR fFf"
    style="background: #0e0c0c"
  >
    <!-- lHh Lpr lFf -->
    <QHeader class="app-header dark q-py-sm">
      <QToolbar>
        <QBtn
          flat
          dense
          round
          @click="leftDrawerOpen = !leftDrawerOpen"
          icon="r_menu"
          size="md"
          class="toggle-main-drawer"
          color="white"
        />

        <QToolbarTitle class="app-title" style="overflow: inherit">
          <QImg
            v-if="!mobileView"
            src="images/excelitas_logo.png"
            fit="fill"
            width="144px"
            height="54px"
            style="min-width: 144px; margin-top: -6px;"
          />
          <span class="q-ml-md text-etc-green" :style="`font-size: 2em; font-family: sap72-bold; letter-spacing: -4px; margin-left: ${(mobileView ? 0 : 24)}px`"
            >XMAP</span
          >
        </QToolbarTitle>
        <QSpace v-if="mobileView" />
        <QBtn
          id="notifs"
          color="white"
          round
          flat
          icon="o_notifications"
          size="md"
          class="app-btn"
          @click="ShowRightDrawerItem(2)"
        >
          <QTooltip>Notifications</QTooltip>
          <QBadge
            v-if="notifs.count > 0"
            color="red"
            floating
            transparent
            rounded
          >
            {{ notifs.count }}
          </QBadge>
        </QBtn>

        <QList class="q-mr-sm">
          <QItem clickable class="q-pa-none q-px-sm">
            <QItemSection avatar :style="`padding-right: 16px; min-width: 32px; ${(mobileView ? 'margin-right: -32px;' : '')}`">
              <QAvatar size="32px">
                <img v-if="user.ActiveUser.Photo" :src="`data:image/png;base64,${user.ActiveUser.Photo}`" />
                <QIcon v-else name="o_face" size="24px" color="green" />
              </QAvatar>
            </QItemSection>
            <QItemSection v-if="user.ActiveUser && !mobileView">
              <QItemLabel class="text-white" style="text-transform: capitalize">{{ user.ActiveUser.DisplayName }}</QItemLabel>
              <QItemLabel v-if="user.ActiveUser.Title" caption class="text-grey" style="text-transform: capitalize; margin-top: 0px">{{ user.ActiveUser.Title }}</QItemLabel>
              <QItemLabel v-if="user.ActiveUser.RoleName" caption class="text-grey" style="text-transform: capitalize; margin-top: 0px">{{ user.ActiveUser.RoleName }}</QItemLabel>
            </QItemSection>

            <QPopupProxy v-if="user.ActiveUser" style="max-width: 100%">
              <QCard style="width: 380px">
              <QCardSection class="q-pa-sm">
                <QList>
                  <QItem>
                    <QItemSection avatar>
                      <QAvatar size="88px">
                        <img
                          v-if="user.ActiveUser.Photo"
                          :src="`data:image/png;base64,${user.ActiveUser.Photo}`"
                        />
                      </QAvatar>
                    </QItemSection>
                    <QItemSection top>
                      <QItemLabel class="text-h6 text-bold">{{
                        user.ActiveUser.DisplayName
                      }}</QItemLabel>
                      <QItemLabel caption>{{
                        user.ActiveUser.Email
                      }}</QItemLabel>
                      <QItemLabel caption>{{
                        user.ActiveUser.Title
                      }}</QItemLabel>

                      <QItemLabel caption
                        >Reports to: {{ user.ActiveUser.Manager }}</QItemLabel
                      >
                    </QItemSection>
                  </QItem>
                </QList>
              </QCardSection>
            </QCard>
            </QPopupProxy>
          </QItem>
        </QList>
      </QToolbar>
    </QHeader>

    <QDrawer
      v-if="user.ActiveUser"
      v-model="leftDrawerOpen"
      show-if-above
      bordered
      :mini="miniState"
      class="app-drawer"
      :width="290"   
      @mouseenter="miniState = false"
      @mouseleave="miniState = true"   
      mini-to-overlay
    >
    <!-- behavior="mobile" -->
      <QList
        class="q-px-none"
        style="
          height: calc(100vh - 155px);
          overflow-y: auto;
          overflow-x: hidden;
        "
      >
        <template v-for="(m, i) in locUserMenu" :key="i">
          <QExpansionItem
            :id="`drawermenuitem${i}`"
            v-if="
              m.label !== 'separator' &&
              (m.userItem !== undefined ||
                (m.hasHeader === undefined &&
                  m.items !== undefined &&
                  m.ignoreItems === undefined))
            "
            group="menu"
            :key="`${i}-ei`"
            :default-opened="m.opened !== undefined ? m.opened : false"
            :icon="m.icon !== '' ? m.icon : 'r_settings'"
            :label="GetLocaleString(m.langkey, m.label)"
            header-class="sticky-menu-header"
            @mouseenter="m.showMenu = true"
            @mouseleave="m.showMenu = false"
            :class="{
              active:
                m.link === undefined
                  ? false
                  : (route.fullPath.indexOf(m.link) > -1 &&
                      miniState &&
                      m.link !== '/') ||
                    (m.link === '/' && route.fullPath === m.link && miniState),
            }"
          >
            <!--  -->
            <QMenu
              v-model="m.showMenu"
              v-show="miniState"
              v-if="miniState"
              auto-close
              @mouseenter="m.showMenu = true"
              @mouseleave="m.showMenu = false"
              touch-position
              :target="`#drawermenuitem${i}`"
              anchor="top right"
              self="top left"
              style="
                background: transparent;
                padding: 8px;
                padding-top: 0px;
                box-shadow: none;
              "
            >
              <QCard class="drawer-pop-menu">
                <QCardSection class="q-pa-sm">
                  <template v-for="(n, o) in m.items">
                    <QItemLabel
                      v-if="n.isHeader !== undefined"
                      :key="o"
                      header
                      style="padding-left: 26px; line-height: 2px !important"
                      >{{ GetLocaleString(n.langkey, n.label) }}</QItemLabel
                    >
                    <QItem
                      v-else
                      :key="`${o}-e`"
                      dense
                      clickable
                      style="padding-left: 26px"
                      class="drawer-item"
                      :active="
                        n.link !== undefined
                          ? route.fullPath.indexOf(n.link) > -1 && n.link !== '/'
                          : false
                      "
                      @click="
                        n.action !== undefined
                          ? handleAction(n.action)
                          : navigateTo({ ...n.route })
                      "
                    >
                      <QItemSection
                        avatar
                        style="width: 32px; min-width: 32px"
                      >
                        <QIcon
                          size="16px"
                          :name="
                            n.icon !== '' && n.icon !== undefined
                              ? n.icon
                              : 'r_settings'
                          "
                        />
                      </QItemSection>
                      <QItemSection>
                        <QItemLabel>{{
                          GetLocaleString(n.langkey, n.label)
                        }}</QItemLabel>
                      </QItemSection>
                    </QItem>
                  </template>
                </QCardSection>
              </QCard>
            </QMenu>

            <QCard style="background-color: transparent">
              <QCardSection style="padding: 0px">
                <QList>
                  <template v-for="(n, o) in m.items">
                    <QItemLabel
                      v-if="n.isHeader !== undefined && miniState"
                      :key="o"
                      header
                      style="padding-left: 26px; line-height: 2px !important"
                      >{{ GetLocaleString(n.langkey, n.label) }}</QItemLabel
                    >
                    <QItem
                      v-else-if="!n.isHeader"
                      :key="`${o}-qe`"
                      clickable
                      style="padding-left: 26px"
                      class="drawer-item"
                      :active="
                        n.link !== undefined
                          ? route.fullPath.indexOf(n.link) > -1 &&
                            !miniState &&
                            n.link !== '/'
                          : false
                      "
                      @click="
                        n.action !== undefined
                          ? handleAction(n.action)
                          : navigateTo({ ...n.route })
                      "
                    >
                      <QItemSection
                        avatar
                        style="width: 32px; min-width: 32px"
                      >
                        <QIcon
                          size="16px"
                          :name="
                            n.icon !== '' && n.icon !== undefined
                              ? n.icon
                              : 'r_settings'
                          "
                        />
                      </QItemSection>
                      <QItemSection>
                        <QItemLabel>{{
                          GetLocaleString(n.langkey, n.label)
                        }}</QItemLabel>
                      </QItemSection>
                    </QItem>
                  </template>
                </QList>
              </QCardSection>
            </QCard>
          </QExpansionItem>

          <QItem
            v-else-if="
              m.label !== 'separator' &&
              (m.items === undefined || m.ignoreItems)
            "
            :key="`${i}-we`"
            clickable
            @click="navigateTo({ ...m.route })"
            :class="{
              'drawer-item': true,
              active:
                m.link === undefined
                  ? false
                  : (route.fullPath.indexOf(m.link) > -1 && m.link !== '/') ||
                    (m.link === '/' && route.fullPath === m.link),
              'mini-active':
                m.link === undefined
                  ? false
                  : (route.fullPath.indexOf(m.link) > -1 &&
                      miniState &&
                      m.link !== '/') ||
                    (route.fullPath === m.link && m.link === '/' && miniState),
            }"
          >
            <QItemSection avatar>
              <QIcon :name="m.icon !== '' ? m.icon : 'r_settings'" />
            </QItemSection>
            <QItemSection>
              <QItemLabel>{{
                GetLocaleString(m.langkey, m.label)
              }}</QItemLabel>
            </QItemSection>

            <QTooltip v-show="miniState && q.screen.width >= 1014" v-if="miniState && q.screen.width >= 1014">{{
              GetLocaleString(m.langkey, m.label)
            }}</QTooltip>
          </QItem>

          <QSeparator v-else :key="`${o}-ee`" />
        </template>
      </QList>
    </QDrawer>

    <QDrawer
      v-model="rightDrawerOpen"
      bordered
      :breakpoint="50000"
      :width="q.screen.width < 400 ? q.screen.width : 400"
      class="app-drawer-right"
      side="right"
    >
      <Notifications
        v-if="rightDrawerItem === 2"
        @close="rightDrawerOpen = false"
        backicon="o_close"
      />
    </QDrawer>

    <QPageContainer
      :class="{ 'app-container': true, mobile: mobileView }"
      style="position: fixed; top: 0px; right: 0px; left: 0px"
    >
      <router-view></router-view>
    </QPageContainer>

    <!-- <q-dialog
      v-model="mainStore.Installable"
      seamless
      :position="mobileView ? 'top' : 'bottom'"
    >
      <QCard class="bg-primary">
        <QCardSection class="row">
          <div style="line-height: 36px; color: white">Install this app?</div>
          <QSpace />
          <QBtn
            label="Dismiss"
            flat
            @click="dismissInstall"
            text-color="white"
          />
          <QBtn
            label="Install"
            color="green"
            @click="installPWA"
            text-color="white"
            unelevated
          />
        </QCardSection>
      </QCard>
    </q-dialog> -->

    <!-- <q-dialog
      v-model="mainStore.hasNewUpdates"
      seamless
      :position="mobileView ? 'top' : 'bottom'"
    >
      <QCard class="bg-red">
        <QCardSection class="row">
          <div style="line-height: 36px; color: white">
            A new version of this app is now available. Please refresh the app.
          </div>
          <QSpace />
          <QBtn
            label="Dismiss"
            flat
            text-color="white"
            @click="mainStore.SetHasNewUpdates(false)"
          />
          <QBtn label="Refresh" text-color="green" flat @click="RefreshApp" unelevated />
        </QCardSection>
      </QCard>
    </q-dialog> -->
  </QLayout>
</template>

<style lang="scss">
.app-container {
  &.mobile {
    bottom: 60px;
    padding-bottom: 20px !important;
    height: inherit;
  }
}
.bottom-menu {
  .q-dialog__backdrop {
    margin-bottom: 70px;
  }
  .q-dialog__inner {
    top: 130px;
    // bottom: 70px;
  }
}
.QExpansionItem--standard {
  &.active {
    .sticky-menu-header {
      // background: rgba(94,94,94, 0.3);
      // border-radius: 32px;
      color: var(--q-positive);
    }
  }
}
.footer-menu {
  .q-tab__icon {
    margin-top: -8px;
  }
}
</style>

<script>
import {
  ref,
  computed,
  onMounted,
  watch,
  provide,
  inject,
  onBeforeUnmount,
  defineAsyncComponent,
} from "vue";
import { useQuasar, Cookies } from "quasar";

import { useRoute } from "vue-router";
import general from "../mixins/general";
import api from "../api/index";
import { useNotifs } from "../store/notifs";
import { useUser } from "../store/user";
import { useLocale } from "../store/locale";
import { useMainStore } from "../store/index";
//import { useOptions } from "../store/options";

export default {
  name: "LayoutDefault",
  components: {
    Notifications: defineAsyncComponent(() =>
      import("../components/General/Notifications.vue")
    ),
  },
  setup() {
    const eventBus = inject("eventBus");
    const GetLocaleString = inject("GetLocaleString");
    const CheckAuth = inject("CheckAuth");

    const q = useQuasar();
    const route = useRoute();
    const { navigateTo, UseLightTheme, toggleDarkMode, NotifyUser } = general();
    const notifs = useNotifs();
    const user = useUser();
    const locale = useLocale();
    const mainStore = useMainStore();
    //const options = useOptions();

    const mobileWidth = ref(650);

    //Vars
    const offlineNotif = ref(null);
    const miniState = ref(true);
    const tab = ref("confirm");
    const rightDrawerOpen = ref(false);
    const locUserMenu = ref([]);
    const rightDrawerItem = ref(1);
    //Computed
    const scrWidth = computed(() => q.screen.width);
    const mobileView = computed(() => {
      var x = q.screen.width < mobileWidth.value;
      if (user.ActiveUser && user.ActiveUser.MobileViewOnly) x = true;
      //x = true; //remove later
      return x;
    });
    const darkMode = computed(() => {
      return q.dark;
      //return !UseLightTheme(); // q.dark;
    });

    const userMenu = computed(() => user.Menu);

    const footerDialog = ref(false);

    const connectionCheck = ref(null);

    provide("mobileWidth", mobileWidth);
    provide("mobileView", mobileView);
    provide("darkMode", darkMode);

    //Watchers
    watch(tab, () => {
      if (tab.value !== "menu") footerDialog.value = false;
    });

    // watch(miniState, () => {
    //   window.localStorage.setItem(
    //     "finxminimenu",
    //     miniState.value ? "1" : "0"
    //   );
    // });

    watch(
      () => userMenu.value,
      () => {
        locUserMenu.value = [...userMenu.value];
      },
      { deep: true, immediate: true }
    );

    watch(rightDrawerItem, () => {
      if (rightDrawerItem.value !== 2)
        window.localStorage.setItem(
          "finxRightDrawerItem",
          rightDrawerItem.value
        );
    });

    watch(() => mainStore.hasNewUpdates, (val) => {
      console.warn("new updates", route, val, mainStore.appNewUpdates)
      if (val) {
        AutoRefreshApp();
      }
    });

    //Methods

    const SwitchTheme = () => {
      toggleDarkMode();
      q.dark.set(!UseLightTheme());
    };

    const handleAction = (data) => {
      if (data.name === "Logout") {
        user.SetUser(null);
        q.cookies.remove("_xmap_auth");
        q.cookies.remove("_pdcs_oss");
        navigateTo({ name: "Login" });
        api.UserLogout();
      } else if (data.name === "ToggleDarkMode") {
        SwitchTheme();
      }
    };

    const installPWA = () => {
      if (mainStore.InstallPrompt) {
        mainStore.InstallPrompt.prompt();
        mainStore.SetInstallable(false);
      }
    };

    const dismissInstall = () => {
      mainStore.SetInstallable(false);
      q.cookies.set("install-xmap-pwa", null, { expires: 15 });
    };

    const RefreshApp = () => {
      mainStore.SetHasNewUpdates(false);
      if (!mainStore.appNewUpdates || !mainStore.appNewUpdates.waiting) return;

      try {
        mainStore.appNewUpdates.waiting.postMessage({ type: "SKIP_WAITING" });
      } catch {
        console.error("Error refreshing app");
        location.reload();
      }
    };

    const ShowRightDrawerItem = (_index) => {
      rightDrawerItem.value = _index;
      rightDrawerOpen.value = true;
    };

    const AutoRefreshApp = () => {
      NotifyUser(
        {
          success: false,
          message: GetLocaleString('UPDATE_NOTIF', `We're almost there! The app is being updated to the latest version for you.`)
        },
        undefined,
        { timeout: 1200, progress: true, position: 'top' }
      );

      setTimeout(() => {
        RefreshApp();
      }, 1000);
    }

    onMounted(async () => {
      console.warn(route)

      // var x = UseLightTheme();
      // q.dark.set(!x);

      setTimeout(() => {
        CheckAuth();
      }, 100);

      if (route.query && route.query.snapmonth !== undefined && route.query.snapyear !== undefined){
        Cookies.remove("_morrdr");
        // mainStore.SyncSessionOptions({
        //   Enabled: true,
        //   Month: parseInt(route.query.snapmonth),
        //   Year: parseInt(route.query.snapyear)
        // });
        navigateTo({name: "Home"});
      }
    });

    onBeforeUnmount(() => {
      clearInterval(connectionCheck);
    });

    return {
      //Vars
      q,
      rightDrawerOpen,
      leftDrawerOpen: ref(false),
      footerDialog,
      tab,
      route,
      offlineNotif,
      notifs,
      mobileWidth,
      locUserMenu,
      rightDrawerItem,

      //Computed
      mobileView,
      user,
      mainStore,
      miniState,
      scrWidth,
      locale,
      darkMode,

      //Methods
      navigateTo,
      handleAction,
      installPWA,
      dismissInstall,
      SwitchTheme,
      GetLocaleString,
      RefreshApp,
      ShowRightDrawerItem,
      eventBus,
      AutoRefreshApp
    };
  },
};
</script>
