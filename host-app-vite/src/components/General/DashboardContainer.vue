...
<template>
  <BoardLayout class="dashboard" :title="GetLocaleString('DASHBOARD', 'Dashboard')" :drawer="drawer" :view="`hHr lpR fFr`" :breakpoint="1024" :show-if-above="false" :drawer-width="450"
    ...
    <template v-slot:header>
      <QToolbar v-if="!invalidSession" class="dashboard-toolbar">
        ...
        <QBtn v-else flat :label="computeTitle" @click="ShowSessionOpts" style="font-size: 1.4rem;font-weight: bold" padding="xs md" />
        <!-- <QToolbarTitle v-else class="text-bold" style="overflow: initial">
          <QBtn flat :label="computeTitle" @click="ShowSessionOpts" style="font-size: 1.4rem;font-weight: bold" padding="xs md" />
        </QToolbarTitle> -->
        ...
        <q-select v-if="mainStore.SessionOptions.Consolidated && user.ActiveUser.IsAdmin" v-model="regionSettings" :options="mainStore.SiteRegions" borderless class="admin-select-period q-mx-sm"
          ...
          <QTooltip>Switch data display</QTooltip>
        </q-select>
        ...
        <QSeparator v-if="mainStore.SessionOptions.Consolidated && user.ActiveUser.IsAdmin" vertical class="q-my-sm" />
        ...
        <QBtn
          ...
          <QTooltip>Compare mode</QTooltip>
        </QBtn>

        <QBtn v-if="[1, 3].indexOf(user.ActiveUser.RoleId) > -1" round flat :icon="mainStore.SessionOptions.Consolidated ? 'r_admin_panel_settings' : 'o_admin_panel_settings'" size="md"
          class="app-btn q-mx-sm" @click="ToggleAdminView">
          <QTooltip>{{ `${(mainStore.SessionOptions.Consolidated ? "Consolidated view" : "Site view")}` }}</QTooltip>
        </QBtn>

        <QSeparator v-if="[1, 3].indexOf(user.ActiveUser.RoleId) > -1" vertical class="q-my-sm" />

        <QBtn v-if="editMode && mode !== 'bowler' && !mainStore.SessionOptions.Consolidated" round flat :icon="fullEdit ? 'o_unfold_less' : 'o_unfold_more'" size="md" class="app-btn q-ml-sm"
          @click="ToggleFullEdit">
          <QTooltip>Toggle Full View</QTooltip>
        </QBtn>

        <QBtn v-if="mode !== 'bowler'" id="btnShowSnapshot" round flat :icon="editMode ? 'o_analytics' : 'o_edit_note'" size="md" class="app-btn q-ml-sm" @click="ToggleView">
          <QTooltip>Edit or Chart View</QTooltip>
        </QBtn>

        <QBtn id="btnRefresh" round flat icon="o_refresh" size="md" class="app-btn q-ml-sm" @click="RefreshReport">
          <QTooltip>Refresh report</QTooltip>
        </QBtn>

        <QBtn v-if="countView > 1" round flat icon="o_visibility">
          <QMenu v-model="showViewer" anchor="top left" self="top right" :offset="[190, -40]">
            <DashboardViewers ref="dashboardViewers" />
          </QMenu>
          <QBadge class="badge-viewer" floating rounded color="green" style="margin-top: 8px">{{ countView }}</QBadge>
          <QTooltip v-if="!showViewer">{{
            `${countView} currently viewing`
          }}</QTooltip>
        </QBtn>

        <slot name="export">
          <QBtn round flat icon="o_get_app" size="md" class="app-btn q-ml-sm" @click="ctx.emit('export')">
            <QTooltip>Export data to Excel</QTooltip>
          </QBtn>
        </slot>
    ...
    <QCardSection v-if="!invalidSession" ref="contdiv" class="container-section q-pa-none" style="height: calc(100vh - 130px);padding: 16px">
    ...
    <QDialog v-model="invalidSession" persistent>
      <QCard>
        <QToolbar>
          ...
          <QBtn flat round icon="o_close" @click="navigateTo({ name: 'Home' })" />
        </QToolbar>
        <QCardSection>
        ...
        <QCard-actions class="row q-py-md" style="justify-content: center;">
