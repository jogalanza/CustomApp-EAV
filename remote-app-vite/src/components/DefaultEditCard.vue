    ...
    <QList class="q-mb-lg">
      ...
      <template v-for="(r, i) in config.rowItemConfig.filter(e => e.view === undefined || (e.view && e.view.indexOf(1) > -1)).sort((a, b) => (a.sortIndex || 0) - (b.sortIndex || 0))" :key="i">
        <template v-if="r.group !== undefined && r.group.length > 0">
          ...
          <QItem :key="i" :class="`q-pb-none ${(r.type === 'text' && i > 0 ? 'q-pt-none' : '')} ${(r.itemclass || '')}`" dense>
            <QItemSection v-for="(rr, ii) in r.group" :key="`${i}${ii}`">
              <QInput v-if="rr.type === 'input' && !rr.readonly" v-model="current[rr.key]" hide-bottom-space dense :label="rr.label" type="number" @blur="HandleBlankValue(rr.key)" input-class="card-input-field" :readonly="CalcReadOnly" />
        ...
        <QItem v-else :key="i" :class="`q-pb-none ${(r.type === 'text' && i > 0 ? 'q-pt-none' : '')} ${(r.itemclass || '')}`" dense>
          <QItemSection>
            <QInput v-if="r.type === 'input' && !r.readonly" v-model="current[r.key]" hide-bottom-space dense :label="r.label" type="number" @blur="HandleBlankValue(r.key)" input-class="card-input-field"
