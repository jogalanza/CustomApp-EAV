<template>
  <div>
    <QCard :class="`multiview-card ${(view === 'grid' ? 'grid-mode' : '')}`" :flat="view === 'grid'">
      <q-tab-panels v-model="localView" style="height: 100%">
        ...
        <q-tab-panel name="default" class="q-pa-none" style="padding: 0px">
          <QCarousel v-model="localSlide" transition-prev="slide-right" transition-next="slide-left" animated style="height: 100%">
            <QCarouselSlide name="history" class="q-pa-none">
              <slot name="history-toolbar">
                <QToolbar class="chart-toolbar">
                  <div class="card-title">{{ `${title}: History` }}</div>
                  <QBtn flat icon="o_arrow_forward" dense @click="localSlide = 'main'" />
                </QToolbar>
              </slot>
              <slot name="history">History</slot>
            </QCarouselSlide>
            <QCarouselSlide name="main" class="q-pa-none">
              <slot name="default-toolbar">
                <QToolbar class="chart-toolbar">
                  ...
                  <!-- <QBtn v-if="dirty" flat icon="r_save" color="green" no-wrap no-caps dense @click="InvokeSave"><QTooltip>Save Changes</QTooltip></QBtn> -->
                  <QBtn flat icon="o_refresh" dense @click="InvokeRefresh">
                    <QTooltip>{{ `${(dirty ? 'Cancel changes' : 'Refresh')}` }}</QTooltip>
                    <QBadge rounded floating color="red" />
                  </QBtn>
                  <QBtn v-if="extramenu && extramenu.length > 0" icon="o_more_vert" flat dense>
                    <QMenu auto-close>
                      <QList>
              ...
              <QToolbar v-if="dirty">
                <QSpace />
                <QBtn  color="green" :label="`Save changes`" no-wrap no-caps @click="InvokeSave" unelevated><QTooltip>Save Changes</QTooltip></QBtn>
              </QToolbar>
            </QCarouselSlide>
          </QCarousel>
        </q-tab-panel>
