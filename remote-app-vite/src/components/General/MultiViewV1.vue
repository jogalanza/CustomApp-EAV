    ...
    <QCard :class="`multiview-card ${(view === 'grid' ? 'grid-mode' : '')}`" :flat="view === 'grid'">
      <q-tab-panels v-model="localView">
        <q-tab-panel name="chart" class="q-pa-none" style="padding: 0px">
          <slot name="chart">
            <GraphCard ref="graphcard" :options="graphopts" />
          </slot>
        </q-tab-panel>
          ...
          <QCarousel v-model="localSlide" transition-prev="slide-right" transition-next="slide-left" animated style="height: 100%">
            <QCarouselSlide name="history" class="q-pa-none">
              <slot name="history-toolbar">
                <QToolbar>
                  ...
                  <QBtn round flat icon="o_arrow_forward" dense @click="localSlide = 'main'" />
                </QToolbar>
              </slot>
            ...
            <QCarouselSlide name="main" class="q-pa-none">
              <slot name="default-toolbar">
                <QToolbar>
                  <QBtn round flat icon="o_history" dense @click="localSlide = 'history'"><QTooltip>History</QTooltip></QBtn>
                  <div class="card-title">{{ title }}</div>
                  <QBtn v-if="dirty" round flat icon="o_save" dense @click="InvokeSave"><QTooltip>Save Changes</QTooltip></QBtn>
                  <QBtn round flat icon="o_refresh" dense @click="InvokeRefresh"><QTooltip>Refresh</QTooltip></QBtn>
                </QToolbar>
              </slot>
