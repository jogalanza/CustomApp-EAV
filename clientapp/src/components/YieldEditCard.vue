      ...
      <template v-for="(r, i) in current.lineItems" :key="i">
        <q-tr>
          ...
          <q-td class="q-py-none">
            <q-input v-model="r.totalUnitsStarted" hide-bottom-space dense type="number" input-class="card-input text-center" :readonly="CalcReadOnly" />
          </q-td>
          <q-td class="q-py-none">
            <q-input v-model="r.totalUnitsProducedGoodFirstTime" hide-bottom-space dense type="number" input-class="card-input text-center" :readonly="CalcReadOnly" />
          </q-td>
          <q-td class="q-py-none">
            <q-input v-model="r.totalUnitsProduced" hide-bottom-space dense type="number" input-class="card-input text-center" :readonly="CalcReadOnly" />
          </q-td>
    ...
    <q-list class="q-mb-lg">
      ...
      <template v-for="(r, i) in config.rowItemConfig.filter(e => e.view === undefined || (e.view && e.view.indexOf(1) > -1)).sort((a, b) => (a.sortIndex || 0) - (b.sortIndex || 0))" :key="i">
        <template v-if="r.group !== undefined && r.group.length > 0">
          ...
          <q-item :key="i" :class="`q-pb-none ${(r.type === 'text' && i > 0 ? 'q-pt-none' : '')} ${(r.itemclass || '')}`" dense>
            <q-item-section v-for="(rr, ii) in r.group" :key="`${i}${ii}`">
              <q-input v-if="rr.type === 'input' && !rr.readonly" v-model="current[rr.key]" hide-bottom-space dense :label="rr.label" type="number" input-class="card-input-field"
        ...
        <q-item v-else :key="i" :class="`q-pb-none ${(r.type === 'text' && i > 0 ? 'q-pt-none' : '')} ${(r.itemclass || '')}`" dense>
          <q-item-section>
            <q-input v-if="r.type === 'input' && !r.readonly" v-model="current[r.key]" hide-bottom-space dense :label="r.label" type="number" input-class="card-input-field" :readonly="CalcReadOnly" />
      ...
      <q-item v-if="resultMsg" :class="`q-px-sm q-mt-md q-mx-sm ${resultMsg.success ? 'bg-primary' : 'bg-negative'}`" dense style="overflow: auto; min-height: 60px; z-index: 10; border-radius: 4px;">
        <q-item-section>
          <q-item-label class="text-white q-pa-sm">{{ resultMsg.message }}</q-item-label>
        </q-item-section>
        <q-item-section side class="q-pa-none">
