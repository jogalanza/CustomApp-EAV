<template>
  <q-card flat class="snapshot-notif-recipients">
    <q-toolbar>
      <q-input
        v-model="search"
        filled
...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    const GetMaster = () => {
      ...
      api
        .SnapshotEmailRecipientsGetAll({ search: search.value })
        .then((r) => {
    ...
    const DeleteItem = (id) => {
      api.SnapshotEmailRecipientsDelete(id).then((response) => {
    ...
    const SelectEmployee = (data) => {
      ...
      if (data.ID !== undefined) {
        api.SnapshotEmailRecipientsSave(data.ID).then((response) => {
          if (response.data.success) {
            GetMaster();
            ctx.emit("update");
          }
