<template>
    <q-card class="update-logs">
        <q-toolbar>
            <q-toolbar-title class="q-pl-md text-bold" style="padding-left: 16px;">Update Logs</q-toolbar-title>
            <!-- <q-input v-model="pagination.search" filled dense class="toolbar-input" style="height: 30px" placeholder="Search ..." clearable :disable="loading" :debounce="800">
            </q-input> -->

            <q-badge rounded :color="HubConnected ? 'green' : 'red'" class="q-mr-sm" style="width: 18px; height: 18px;">
              <q-tooltip>{{ `${(HubConnected ? 'Online' : 'Offline')}` }}</q-tooltip>
            </q-badge>

            <q-btn v-if="!collapseMenu && !loading && !mobileView" round flat icon="r_refresh" size="md" @click="GetMaster" no-caps stack>
                <q-tooltip>Refresh</q-tooltip>
            </q-btn>

            <q-btn flat round icon="o_upload_file" no-caps stack
                @click="CheckUploadAction" />

            <q-btn flat round icon="o_download" no-caps stack @click="Download" />
        ...
        <q-toolbar>
          <q-list>
            <q-item>
              <q-item-section>
                <q-item-label v-if="apiVer.Build || apiVer.Version">API version {{ `${apiVer.Build || apiVer.Version}` }}</q-item-label>
                <q-item-label>UI version {{ appVer }}</q-item-label>
              </q-item-section>
            </q-item>
          </q-list>
        </q-toolbar>
        <q-card-section :class="`q-py-none ${(mobileView ? 'q-px-none' : '')}`" style="height: calc(100% - 120px)">
        <q-table flat :class="`app-data-grid sticky-header sticky-row-header sticky-action-col ${(mobileView ? 'dense-pagination' : '')}`" :columns="columns" :rows="items" row-key="ID"
          ...
          <template v-slot:item="props">
            <q-card class="grid-item q-mb-sm" style="width: 100%; margin-top: 2px;" clickable>
              <q-card-section class="q-pa-none" style="cursor: pointer">
                <div :class="{
    'q-item row q-pa-md q-item--clickable': true,
    'no-wrap': false,
  }">
                  <!-- <q-item-section avatar top style="justify-content: inherit; align-items: center">
                    <ProfileAvatar :text1="props.row.PackageName" listmode :selected="props.row.isSelected" />
                  </q-item-section> -->

                  <q-item-section class="row wrap q-pm-md" style="justify-content: flex-start">
                    <q-item-label :class="`text-bold`">{{ props.row.PackageName }}</q-item-label>
                    <q-item-label caption style="color: inherit">Prior version: {{ props.row.PriorVersion }}</q-item-label>
                    <q-item-label caption style="color: inherit">Processed by {{ props.row.UserName }}</q-item-label>
                    <q-item-label caption style="color: inherit">Updated on {{ props.row.Timestamp }}</q-item-label>
                    
                  </q-item-section>

                  <!-- <q-item-section side top>
                    <q-btn v-if="IsMenuOK('ADM_UPD_EDIT')" flat round icon="o_edit" @click="EditItem(props.row)" color="grey" size="sm" />
                  </q-item-section> -->
                </div>
              </q-card-section>
            </q-card>
          ...
          <template v-slot:body-cell="props">
            ...
            <q-td v-else-if="props.col.field === 'PackageName'" :props="props">
              <div class="q-item q-pa-none row no-wrap">
                <!-- <q-item-section avatar>
                  <ProfileAvatar :text1="props.row.PackageName" listmode :selected="props.row.isSelected" />
                </q-item-section> -->
                <q-item-section class="text-left" style="font-size: 0.8rem">
                  <div class="text-bold" style="font-size: 1em;">{{ props.row.PackageName }}</div>
                  <q-item-label caption style="color: inherit">{{ props.row.UserName }}</q-item-label>
                  <q-item-label caption style="color: inherit">{{ props.row.Timestamp }}</q-item-label>
                </q-item-section>
              </div>
            </q-td>
