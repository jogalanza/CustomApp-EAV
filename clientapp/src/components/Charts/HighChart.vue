<template>
  <q-card class="chart-card" :style="`width:${width}`">
    <!-- max-height:600px -->
    <q-toolbar class="chart-toolbar">
      <q-btn v-if="level > 1" icon="mdi-arrow-left" dense rounded flat @click="ChartLevelUp">
        <q-tooltip>Chart one level up</q-tooltip>
      </q-btn>
      <q-toolbar-title style="font-size:16px">{{ ActiveTitle }}</q-toolbar-title>

      <q-btn v-if="showToolbarControl" icon="mdi-refresh" dense rounded flat @click="RefreshChart">
        <q-tooltip>Refresh</q-tooltip>
      </q-btn>
      <q-btn v-if="showToolbarControl" icon="mdi-download" dense rounded flat >
        <q-menu :offset="[130, 0]" :dark="GetDarkModeBeta">
          <q-list :dark="GetDarkModeBeta">
            <q-item dense clickable @click="DownloadChart" :dark="GetDarkModeBeta">
              <q-item-section class="qmenu-text">
                Download Chart as Image
              </q-item-section>
            </q-item>
            <q-item dense clickable @click="DownloadData" :dark="GetDarkModeBeta"> 
              <q-item-section class="qmenu-text">
        ...
        <!-- <q-tooltip>Download Chart as Image</q-tooltip> -->
      </q-btn>
      <q-btn v-if="showToolbarControl" icon="mdi-fullscreen" dense rounded flat @click="GoFullScreen">
        <q-tooltip>Fullscreen View</q-tooltip>
      </q-btn>
    </q-toolbar>
    <!-- height:${height}; -->
    <q-card-section :style="`padding-top:0px`">
      ...
      <q-inner-loading :showing="busy" :dark="GetDarkModeBeta">
        <q-item-label class="loading-message">{{ busyMsg }}</q-item-label>
    ...
    <q-dialog persistent maximized v-model="dialog">
      <q-card :class="{'modern-beta-dialog': GetDarkModeBeta}">
        <q-toolbar class="chart-toolbar">
          <q-btn v-if="level > 1" icon="mdi-arrow-left" dense rounded flat @click="ChartLevelUp">
            <q-tooltip>Chart one level up</q-tooltip>
          </q-btn>
          <q-toolbar-title>{{ ActiveTitle }}</q-toolbar-title>
          <q-btn icon="mdi-close" dense rounded flat @click="dialog =  false" />
        </q-toolbar>
        <q-card-section style="height: calc(100vh - 64px);">
