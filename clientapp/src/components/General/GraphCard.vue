  ...
  <q-card flat class="graph-card q-pb-md">
    ...
    <q-toolbar class="chart-toolbar q-py-sm q-px-md">
      <slot name="chart-toolbar">
        ...
        <!-- <q-btn round flat icon="o_download" dense size="12px"><q-tooltip>Download</q-tooltip></q-btn> -->
        <div class="card-title">{{ CalcTitle }}</div>
        <q-btn v-if="dirty" round flat icon="o_save" dense @click="SaveChanges" size="12px"><q-tooltip>Save Changes</q-tooltip></q-btn>
        <q-toggle v-if="options.toggleView && (options.toggleView.visible === undefined || options.toggleView.visible(mainStore.SessionOptions))" v-model="altView" round flat dense size="32px"
          color="orange">
          <q-tooltip>{{ `${(altView ? options.toggleView?.label || 'Switch view' : 'Default view')}` }}</q-tooltip>
        </q-toggle>
        <q-btn v-if="options.externalLink" round flat icon="o_open_in_new" dense @click="OpenLink" size="12px">
          <q-tooltip>{{ `${(options.externalLink?.label || 'Open an external link')}` }}</q-tooltip>
        </q-btn>
        <q-btn round flat icon="o_refresh" dense @click="RefreshChart" size="12px"><q-tooltip>Refresh</q-tooltip></q-btn>
        <q-btn round flat icon="o_fullscreen" dense @click="dialog = !dialog" size="12px"><q-tooltip>Toggle Fullscreen View</q-tooltip></q-btn>

        <q-btn v-if="extramenu && extramenu.filter(e => e.view === undefined || e.view.indexOf(1) > -1).length > 0" icon="o_more_vert" round flat dense>
          <q-menu auto-close>
            <q-list>
              <q-item v-for="(m, i) in extramenu.filter(e => e.view === undefined || e.view.indexOf(1) > -1)" :key="i" clickable @click="m.action" dense>
    ...
    <div v-if="CalcShowComment" class="side-comment col-12 q-pa-sm scroll q-mx-md">
      <q-card-section v-if="!CalcReadOnly" class="q-pa-none row" style="position: absolute; z-index: 2; right: 20px;">
        <q-btn size="9px" icon="o_content_copy" color="#b3a3a3" flat round @click.stop="InvokeCopyPrevious" padding="xs" style="color: #b3a3a3">
          <q-tooltip>Copy from previous period</q-tooltip>
        </q-btn>
      </q-card-section>
      <q-card-section v-if="options.remarkType && options.remarkType.indexOf(1) > -1" class="q-pa-none q-pb-sm">
      ...
      <q-card-section v-if="options.remarkType && options.remarkType.indexOf(2) > -1" class="q-pa-none q-pb-sm">
      ...
      <q-card-section v-if="options.remarkType && options.remarkType.indexOf(3) > -1" class="q-pa-none q-pb-sm">
    ...
    <q-dialog v-model="dialog" persistent>
      <q-card class="graph-card-fs">
        ...
        <q-toolbar class="chart-toolbar">
          ...
          <q-btn v-if="options.externalLink" round flat icon="o_open_in_new" dense @click="OpenLink">
            <q-tooltip>{{ `${(options.externalLink?.label || 'Open an external link')}` }}</q-tooltip>
          </q-btn>
        ...
        <q-card-section v-if="showChart" class="row" style="height: calc(100% - 50px)">
          ...
          <div v-if="CalcShowComment" :class="`side-comment ${(CalcShowComment ? 'col-12 col-sm-4' : 'col-12')} q-pa-md scroll`">
            <q-card-section v-if="!CalcReadOnly" class="q-pa-none row" style="position: absolute; z-index: 2; right: 24px; margin-top: -8px;">
              <q-btn size="14px" icon="o_content_copy" color="#b3a3a3" flat round @click.stop="InvokeCopyPrevious" padding="xs" style="color: #b3a3a3">
                <q-tooltip>Copy from previous period</q-tooltip>
              </q-btn>
            </q-card-section>
            <q-card-section v-if="options.remarkType && options.remarkType.indexOf(1) > -1" class="q-pa-none q-pb-md">
            ...
            <q-card-section v-if="options.remarkType && options.remarkType.indexOf(2) > -1" class="q-pa-none q-pb-md">
            ...
            <q-card-section v-if="options.remarkType && options.remarkType.indexOf(3) > -1" class="q-pa-none q-pb-md">
