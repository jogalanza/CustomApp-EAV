<template>
  <q-dialog v-model="dialog" persistent>
    <q-card class="bg-red text-white">
      <q-toolbar>
          <q-avatar v-if="icon !== null" :icon="icon" text-color="white" size="60px" />
          <q-space />
          <q-btn flat  @click="dialog = false" label="Dismiss" />
        </q-toolbar>
      <q-card-section class="q-pt-none row">
        <div class="text-h6">{{ calcMsg }}</div>
      </q-card-section>
      <q-card-section v-if="calcMsg2 !== null" class="q-pt-none row">
        <q-separator class="col-12 bg-grey q-mb-md" style="height:1px"/>
