  ...
  <BoardLayout class="manage-targets" :loading="loading" :drawer="drawerRight" :breakpoint="20000" :show-if-above="false" :drawer-width="q.screen.width < 450 ? q.screen.width : 450"
    ...
    <q-card flat style="height: 100%; width: 100%">
      <q-toolbar class="text-bold q-pt-sm q-px-lg" style="overflow: initial">
        <q-toolbar-title class="clip text-bold">Period</q-toolbar-title>
        <q-space />
    ...
    GetLocaleString("ADD_NEW_PERIOD", "Add new period")
      ...
      <EditPeriod ref="editPeriod" @update="GetMaster" />

      <q-card-section :class="`${mobileView ? 'q-pa-none' : 'q-pt-none'}`" style="height: calc(100% - 64px)">


        <q-table class="col-12 sticky-header sticky-row-header sticky-action-col" flat :columns="columns" :rows="items" row-key="DESCRIPTION" virtual-scroll v-model:pagination="pagination"
          :visible-columns="visibleCols" :loading="loading" @request="GetMaster" :filter="pagination.search" selection="multiple" v-model:selected="selected" hide-pagination
          ...
          <template v-slot:top>
            <q-toolbar>
              <q-select v-model="pagination.Year" :options="filters.years" label="Show by Year" map-options emit-value dense filled style="min-width: 180px" :disable="loading" />
              <q-space />
            </q-toolbar>
...
export default defineComponent({
  ...
  emits: ["edit", "addnew"],
  components: {
    EditPeriod: defineAsyncComponent(() => import("@/components/Admin/EditPeriod.vue")),
  },
  setup(props, ctx) {
