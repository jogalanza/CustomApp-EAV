<template>
  <BoardLayout class="manage-accounts" :loading="loading" :drawer="drawerRight" :breakpoint="20000" :show-if-above="false" :drawer-width="q.screen.width < 450 ? q.screen.width : 450"
    ...
    <q-card flat style="height: 100%; width: 100%">
      <q-toolbar class="text-bold q-pt-sm q-px-lg" style="overflow: initial">
        <q-toolbar-title class="clip text-bold">Accounts</q-toolbar-title>
        <q-space />
      ...
      <q-card-section :class="`${mobileView ? 'q-pa-none' : 'q-pt-none'}`" style="height: calc(100% - 64px)">
        ...
        <q-table class="col-12 sticky-header sticky-row-header sticky-action-col" flat :columns="columns" :rows="items" row-key="ID" virtual-scroll v-model:pagination="pagination"
          ...
          <template v-slot:top>
            <q-toolbar>
              <q-input v-model="pagination.search" filled borderless dense class="toolbar-input" style="height: 30px" :placeholder="GetLocaleString('SEARCH', 'Search...')" :disable="loading"
                :debounce="800" autofocus>
          ...
          <template v-slot:body-cell-action="props">
            <q-td class="sticky-bg-color" :props="props" :style="props.col.style ? { ...props.col.style } : null">
              <q-btn v-if="[1, 2].indexOf(props.row.RoleId) > -1" icon="o_key" unelevated dense round no-caps @click="CopyAPIKey(props.row)" class="q-mr-sm" size="sm">
...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    const CopyAPIKey = (data) => {
      navigator.clipboard.writeText(data.GUUID);
      NotifyUser({ success: true, message: "API key copied to clipboard" });
    }
