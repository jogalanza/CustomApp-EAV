<template>
  <BoardLayout class="manage-targets" :loading="loading" :drawer="drawerRight" :breakpoint="20000" :show-if-above="false" :drawer-width="q.screen.width < 450 ? q.screen.width : 450"
    ...
    GetLocaleString("ADD_NEW_PERIOD", "Add new period")
      ...
      <EditProduct ref="editPeriod" @update="GetMaster" />

      <q-card-section :class="`${mobileView ? 'q-pa-none' : 'q-pt-none'}`" style="height: calc(100% - 64px)">


        <q-table class="col-12 sticky-header sticky-row-header sticky-action-col" flat :columns="columns" :rows="items" row-key="ProductLineId" virtual-scroll v-model:pagination="pagination"
...
export default defineComponent({
  ...
  emits: ["edit", "addnew"],
  components: {
    EditProduct: defineAsyncComponent(() => import("@/components/Admin/EditProduct.vue")),
  },
  setup(props, ctx) {
    ...
    const DeleteItems = () => {
      ConfirmAction(
        ...
        () => {
          var x = [];
          selected.value.map((e) => x.push(e.ProductLineId));
          loading.value = true;
          api
            .Delete(x)
            .then((response) => {
