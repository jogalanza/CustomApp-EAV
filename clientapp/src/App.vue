...
import { resetTourCookies } from "./composables/appTour";
...
export default {
  ...
  setup() {
    ...
    const ResetGuidedTour = () => {
      resetTourCookies(q).then(() => {
        mainStore.SetAdHocTour(true);
        if (["Dashboard", "Home"].indexOf(route.name) === -1) {
          navigateTo({ name: "Home" });
        } else {
          eventBus.$emit("invoke-guided-tour");
        }
      });
    };
